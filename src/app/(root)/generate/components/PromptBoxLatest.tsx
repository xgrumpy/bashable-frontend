...
function PromptBox({ promptText, setPromptText, handleError }: IPromptBoxProps) {
        ...
        if (username && inputText) {
            axiosReq.get(`/generate/check?prompt=${inputText}`).then((res) => {
                if (res.data.restricted) {
                    handleError("Will be marked as restricted!");
    ...
    }, [handleError, inputText, username]);
    ...
    useEffect(() => {
        if (inputText) {
            ...
            restrictedKeywords.forEach((word: string) => {
                ...
                const replacement = `<span class="break-words border-b-2 border-yellow-600 dark:border-yellow-300">${word}</span>`;
        ...
        <div className="relative z-10">
            <div className="relative border border-borderlight dark:border-border rounded-md px-3 md:px-4 py-2 md:py-3">
                <div className="inner relative">
                    ...
                    <div
                        ref={warningRef}
                        className="relative pointer-events-none min-h-[180px] z-10 !scrollbar-hidden break-words"
