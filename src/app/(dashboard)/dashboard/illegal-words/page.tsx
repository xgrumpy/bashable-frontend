...
const IllegalWordsPage = () => {
    const [modalDeleteId, setModadlDeleteId] = useState<string>("");
    const [modalEditItem, setModalEditItem] = useState<IIllegalWord | null>(
        null
    );
    const [currentWord, setCurrentWord] = useState<string>("");

    const { data: illegalWords, refresh } = useFetch("/admin/illegal_words");

    const handleSubmit = (e: FormEvent) => {
        e.preventDefault();
        axiosReq
            .post("/admin/illegal_words", {
    ...
    const handleEdit = (e: FormEvent) => {
        ...
        if (modalEditItem) {
            axiosReq
                .post(`/admin/illegal_words/${modalEditItem.id}`, {
    ...
    const handleDelete = () => {
        axiosReq
            .delete(`/admin/illegal_words/${modalDeleteId}`)
            .then((res) => {
                setModadlDeleteId("");
                toast.success(res.data.message);
                refresh();
    ...
    return (
        <>
            ...
            <Modal
                state={!!modalEditItem}
                closeHandler={() => setModalEditItem(null)}
            >
...
export default IllegalWordsPage;
